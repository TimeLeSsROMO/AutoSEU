...
def getUserData():
    ...
    # 写入账号密码文件
        ...
        print(" ")
        print("Tips：请确保在现版系统中手动进行过至少一次完整的健康申报和出校审批，否则可能出现奇怪的错误导致无法正常运行")
        print("      各种问题可以联系TimeLeSs，如果你可以联系到我的话~_~")
        print(" ")
        print("Version 1.9 by Someone named TimeLeSs or TimeLeSsROMO")
        print(" ")
...
def sqxj():
    ...
    print('已完成销假，即将开始申请出校')
    time.sleep(2)

def checksqxj2():
    browser.get(enter_campus_url)
    time.sleep(5)
    Context = browser.find_element_by_xpath("/html/body/div[1]/div/div/div[2]/div[1]/div[1]/div[2]/div").text
    global flag
    try:
          if "已销假" in Context:
                return flag
...
def enterCampus(browser):
    ...
    if qt2 == "N":
         ...
         if checkEnterCampus(browser) == True:
              ...
              # 进行销假后才可以申请
              checksqxj()
              if checksqxj() == False and flag == False:
                    browser.close()
                    print("---销假可能需学校人工审核，待收到通知短信后再运行本脚本---")
                    print("-----------------------后台已关闭-------------------------")
                    time.sleep(10)
              elif checksqxj() == False and flag == True:
                    print("无需销假，即将开始明日的出校审批")
                    time.sleep(2)
              while checksqxj() == True:
                    ...
                    checksqxj()
                    time.sleep(5)
                    if checksqxj() == True and flag == True:
                          browser.close()
                          print("---销假可能需学校人工审核，待收到通知短信后再运行本脚本---")
                          print("-----------------------后台已关闭-------------------------")
                          time.sleep(10)
                          ...
                          break
                    elif checksqxj() == False and flag == False:
                          browser.close()
                          print("---销假可能需学校人工审核，待收到通知短信后再运行本脚本---")
                          print("-----------------------后台已关闭-------------------------")
                          time.sleep(10)
              ...
              # 确定年份
              js = "document.querySelector(\"#app > div > div > div > div.emapm-form > div:nth-child(2) > div > div.mint-cell-group-content.mint-hairline--top-bottom.mt-bg-white.mt-bColor-after-grey-lv5 > div:nth-child(5) > div > div.mint-popup.mt-bg-white.mint-datetime.emapm-date-picker.mint-popup-bottom > div > div.mint-picker__columns > div:nth-child(1) > ul > li:nth-child(" + str(int(tomorrow_year)-1920) + ")\").click();"
              browser.execute_script(js)
              time.sleep(1)
              ...
              # 确定年份
              js = "document.querySelector(\"#app > div > div > div > div.emapm-form > div:nth-child(2) > div > div.mint-cell-group-content.mint-hairline--top-bottom.mt-bg-white.mt-bColor-after-grey-lv5 > div:nth-child(6) > div > div.mint-popup.mt-bg-white.mint-datetime.emapm-date-picker.mint-popup-bottom > div > div.mint-picker__columns > div:nth-child(1) > ul > li:nth-child(" + str(int(tomorrow_year)-1920) + ")\").click();"
              browser.execute_script(js)
              time.sleep(1)
              ...
              time.sleep(2)
              try:
                  Context3 = browser.find_element_by_xpath("/html/body/div[4]/div/div[2]/div").text
                  if "存在未销假的请假记录，暂不可请假" in Context3:
                        browser.close()
                        print("---销假可能需学校人工审核，待收到通知短信后再运行本脚本---")
                        print("-----------------------后台已关闭-------------------------")
                        time.sleep(10) 
